...
const WhatsappPopup = ({ setwpPop, email }) => {
  ...
  async function handleclosed() {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/user/closed?email=${email}`,
        {
          method: "POST",
          headers: {
  ...
  async function handlejoin() {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/user/joined?email=${email}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    setwpPop(false);
    window.open("https://chat.whatsapp.com/CfdYJemiPVCA3uQSXj64D2", "_blank");
  }
  return (
    <div className="whatsapp-popup">
      ...
      <div className="desktop-wp-pop">
        Please Open{" "}
        <b style={{ color: "#1aba5b" }}>upskillmafia.com/mern/whatsapp</b> from
        your mobile to join Group Seamlessly
      </div>
      <div
